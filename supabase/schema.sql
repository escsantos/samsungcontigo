...
-- ================================================================
-- CORREÇÃO — Linha do tempo (Rastreabilidade) agora mostra o login que
-- fez cada alteração (criou, revisou/enviou, faturou, liberou sem
-- pagamento, separou, entregou, confirmou recebimento, registrou
-- pagamento, liberou peça). Sem essa política, os joins
-- perfis!orcamentos_..._fkey usados pela tela voltam null pra quem não é
-- Administrador/Diretor/Gerente, porque a RLS de "perfis" só liberava o
-- próprio perfil ou o vendedor do pedido — as demais colunas de autoria
-- (criado_por, revisado_por, pagamento_validado_por,
-- liberado_sem_pagamento_por, separado_por, entregue_por,
-- recebimento_confirmado_por, registrado_por em pagamentos_orcamento,
-- liberado_por em orcamento_itens) não estavam cobertas. As subconsultas
-- abaixo continuam respeitando a RLS de orcamentos/pagamentos/itens, então
-- não abre visibilidade nova — só libera o NOME de quem já aparece como
-- autor em um pedido que o usuário já enxerga.
-- Rode este arquivo inteiro no SQL Editor do Supabase
-- ================================================================
...
-- ================================================================
-- CORREÇÃO — OS Interna: JM3 Cliente não conseguia gravar (a policy
-- "revisar orcamentos" exige vínculo em perfis_unidades E, mesmo tendo
-- esse vínculo, só libera update se vendedor_id = auth.uid() — o que
-- falha pra qualquer pedido em que o login JM3 Cliente que está editando
-- não é o mesmo que criou/é vendedor daquele pedido específico). Em vez
-- de reabrir a policy de UPDATE de orcamentos pra JM3 Cliente (o que
-- devolveria acesso de escrita a status, valores etc — exatamente o que
-- as correções anteriores fecharam), a gravação passa a ser feita por
-- uma função com permissão própria, que só grava a coluna os_interna e
-- carimba quem/quando (pra aparecer na linha do tempo).
-- Rode este arquivo inteiro no SQL Editor do Supabase
-- ================================================================
alter table orcamentos add column if not exists os_interna_por uuid references perfis(id);
alter table orcamentos add column if not exists os_interna_em timestamptz;

create or replace function salvar_os_interna(p_orcamento_id bigint, p_valor text)
...
begin
  select exists (
    ...
    where o.id = p_orcamento_id
      and (
        (eh_jm3_cliente() and o.cliente_id = meu_cliente_id())
        or (
          exists (select 1 from perfis_unidades pu where pu.unidade_id = o.unidade_id and pu.perfil_id = auth.uid())
          and (o.vendedor_id = auth.uid() or pode_ver_todos_orcamentos() or pode_gerenciar_estoque())
        )
  ...
  if not pode then
    raise exception 'Sem permissão para editar a OS Interna deste pedido.';
  end if;

  update orcamentos
  set os_interna = p_valor, os_interna_por = auth.uid(), os_interna_em = now()
  where id = p_orcamento_id;
end;
$$;

grant execute on function salvar_os_interna(bigint, text) to authenticated;

-- E o join perfis!orcamentos_os_interna_por_fkey usado pela linha do
-- tempo também precisa da coluna coberta na policy de nomes:
drop policy if exists "ve nome de quem agiu em pedidos visiveis" on perfis;
create policy "ve nome de quem agiu em pedidos visiveis"
  on perfis for select
  using (
