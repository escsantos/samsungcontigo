...
// Como findExact, mas aceita uma lista de nomes equivalentes pra mesma coluna
// (ex: a planilha às vezes vem com "Qtd", às vezes com "Qtde") — devolve o
// índice do primeiro nome que bater, ou -1 se nenhum bater. Os nomes em
// `alvos` já devem estar em normKey, igual `headers`.
export function findAny(headers, alvos) {
  for (const alvo of alvos) {
    const idx = headers.indexOf(alvo);
    if (idx >= 0) return idx;
  }
  return -1;
}
