...
export default function DetalheOrcamentoPage() {
  ...
  async function carregar() {
    ...
    const { data: orc } = await supabase
      .from("orcamentos")
      .select(`*, clientes(nome, celular, email, cnpj),
        ...
        liberador:perfis!orcamentos_liberado_sem_pagamento_por_fkey(nome),
    ...
    const { data: pags } = await supabase
      ...
      .select("*, registrador:perfis!pagamentos_orcamento_registrado_por_fkey(nome)")
      .eq("orcamento_id", id)
      .order("registrado_em");
    setPagamentos(pags || []);
    if (orc) {
  ...
  async function salvarOsInterna() {
    ...
    const { error } = await supabase.rpc("salvar_os_interna", { p_orcamento_id: Number(id), p_valor: valor });
  ...
  // Valor de venda da linha editável direto (ex: negociar um valor maior/menor
  // com o cliente) — não mexe no custo. O imposto e a margem (por item e no
  // resumo final) são sempre calculados a partir de i.venda_total, então já
  // recalculam sozinhos assim que esse valor muda. Edita o total da linha
  // (não o unitário) pra caber num campo só, numa linha só.
  function mudarVendaItem(itemId, valor) {
    const novoTotal = parseFloat(valor);
    setItens((atual) =>
      atual.map((i) => {
        if (i.id !== itemId) return i;
        const total = isNaN(novoTotal) ? 0 : novoTotal;
        return { ...i, venda_total: total, venda_unitario: i.qtd > 0 ? total / i.qtd : total };
      })
    );
  }
  ...
  return (
    <AppShell titulo={`Orçamento #${orcamento.numero_unidade}`}>
      ...
      <div className="card p-6 mb-4">
          ...
          <div className="flex items-center gap-2 mt-3">
            ...
            {editandoOS ? (
              ...
              <>
                ...
                {["Administrador", "Diretor", "Gerente", "Supervisor", "Vendedor", "JM3 Cliente"].includes(perfil?.cargo) && (
                  <button
        ...
        {orcamento.status !== "Pendente de Análise" && orcamento.status !== "Rejeitado" && ["Administrador", "Diretor", "Gerente", "Supervisor", "JM3 Cliente", "Estoque"].includes(perfil?.cargo) && (
          <button
                  ...
                  <td className="px-3 py-2.5 text-right text-sm">
                    {ajustando ? (
                      <input
                        ...
                        title="Valor de venda da linha editável só pra este pedido — o imposto e a margem recalculam na hora"
                      />
                    ) : (
